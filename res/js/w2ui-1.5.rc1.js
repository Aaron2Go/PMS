...
var w2utils = (function ($) {
    ...
    var obj = {
        ...
        settings: {
            ...
            "shortdays": ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"],
            "fulldays": ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"],
            "weekStarts": "星期一",        // can be "M" for Monday or "S" for Sunday
            "dataType": 'HTTPJSON', // can be HTTP, HTTPJSON, RESTFULL, RESTFULLJSON, JSON (case sensitive)
            "phrases": {},         // empty object for english phrases
            "dateStartYear": 1990,       // start year for date-picker
    ...
    function isDate(val, format, retDate) {
        ...
        if (String(dt) == '无效日期') return false;
    ...
    function date(dateStr) {
        ...
        var dsp = dd1;
        if (dd1 === dd2) dsp = time;
        if (dd1 === dd3) dsp = w2utils.lang('昨日');
...
(function ($) {
    ...
    w2grid.prototype = {
        ...
        buttons: {
            'reload': { type: 'button', id: 'w2ui-reload', icon: 'w2ui-icon-reload', tooltip: '刷新所有数据' },
            'columns': { type: 'drop', id: 'w2ui-column-on-off', icon: 'w2ui-icon-columns', tooltip: '显示/隐藏字段', arrow: false, html: '' },
            'search': {
            ...
            'search-go': { type: 'drop', id: 'w2ui-search-advanced', icon: 'w2ui-icon-search', text: '高级筛选', tooltip: '打开检索字段' },
            'add': { type: 'button', id: 'w2ui-add', text: '增加记录', tooltip: '增加新记录', icon: 'w2ui-icon-plus' },
            'edit': { type: 'button', id: 'w2ui-edit', text: '编辑记录', tooltip: '编辑已选记录', icon: 'w2ui-icon-pencil', disabled: true },
            'delete': { type: 'button', id: 'w2ui-delete', text: '删除记录', tooltip: '删除已选记录', icon: 'w2ui-icon-cross', disabled: true },
            'save': { type: 'button', id: 'w2ui-save', text: '保存更改', tooltip: '保存所有已更改数据', icon: 'w2ui-icon-check' }
        },
        ...
        searchReset: function (noRefresh) {
            ...
            if (this.searches.length > 0) {
                if (!this.multiSearch || !this.show.searchAll) {
                    ...
                    this.last.field = 'all';
                    this.last.caption = w2utils.lang('检索');
                }
            }
        ...
        searchShowFields: function () {
            ...
            for (var s = -1; s < this.searches.length; s++) {
                ...
                if (s == -1) {
                    ...
                    search = { field: 'all', caption: w2utils.lang('默认检索') };
                } else {
        ...
        initAllField: function (field, value) {
            var el = $('#grid_' + this.name + '_search_all');
            if (field == 'all') {
                var search = { field: 'all', caption: w2utils.lang('检索') };
                el.w2field('clear');
                el.change();
            } else {
                var search = this.getSearch(field);
        ...
        "delete": function (force) {
            ...
            if (this.msgDelete != '' && !force) {
                this.message({
                    ...
                    buttons: '<button class="w2ui-btn w2ui-btn-red" onclick="w2ui[\'' + this.name + '\'].delete(true)">' + w2utils.lang('确定') + '</button>' +
                        '<button class="w2ui-btn" onclick="w2ui[\'' + this.name + '\'].message()">' + w2utils.lang('取消') + '</button>',
        ...
        initColumnOnOff: function () {
            ...
            if (obj.show.saveRestoreState) {
                col_html += '<tr><td colspan="2" onclick="var obj = w2ui[\'' + obj.name + '\']; obj.toolbar.uncheck(\'w2ui-column-on-off\'); obj.stateSave();">' +
                    '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">' + w2utils.lang('保存显示设置') + '</div>' +
                    '</td></tr>' +
                    '<tr><td colspan="2" onclick="var obj = w2ui[\'' + obj.name + '\']; obj.toolbar.uncheck(\'w2ui-column-on-off\'); obj.stateReset();">' +
                    '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">' + w2utils.lang('恢复默认设置') + '</div>' +
                    '</td></tr>';
            }
        ...
        getSearchesHTML: function () {
            ...
            html += '<tr>' +
                ...
                '        <div>' +
                '        <button class="w2ui-btn" onclick="obj = w2ui[\'' + this.name + '\']; if (obj) { obj.searchReset(); }">' + w2utils.lang('重置') + '</button>' +
                '        <button class="w2ui-btn w2ui-btn-blue" onclick="obj = w2ui[\'' + this.name + '\']; if (obj) { obj.search(); }">' + w2utils.lang('检索') + '</button>' +
                '        </div>' +
                '    </td>' +
                '</tr></tbody></table>';
            return html;
            ...
            function getNodeHTML(nd) {
                ...
                if (nd.group) {
                    html =
                        ...
                        ((nd.groupShowHide && nd.collapsible) ? '<span>' + (!nd.hidden && nd.expanded ? w2utils.lang('隐藏') : w2utils.lang('显示')) + '</span>' : '<span></span>') +
        ...
        updateOverlay: function (indexOnly) {
            ...
            if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
                ...
                if ($(input).is(':focus')) {
                    ...
                    if (options.url == null && options.items.length === 0) msgNoItems = w2utils.lang('无记录');
        ...
        validate: function (showErrors) {
            ...
            for (var f = 0; f < this.fields.length; f++) {
                ...
                if (field.required && (val === '' || ($.isArray(val) && val.length === 0) || ($.isPlainObject(val) && $.isEmptyObject(val)))) {
                    errors.push({ field: field, error: w2utils.lang('必填') });
                }
